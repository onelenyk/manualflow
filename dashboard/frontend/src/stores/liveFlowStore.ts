...
import { mapInteractionToCommands, getMappingAlternatives, parseCommandLines } from '@maestro-recorder/shared';
...
function parseEnhancedYaml(yaml: string): { appId: string | null; commands: MaestroCommand[] } {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (m) appId = m[1].trim().replace(/^['"]|['"]$/g, '');
    if (t === '---') { bodyStart = i + 1; foundSeparator = true; break; }
  }
  if (!foundSeparator) bodyStart = 0;

  const commands = parseCommandLines(lines.slice(bodyStart).join('\n'));
